...
int rms_values[64] = {
  1000000, 828427, 779763, 756828, 743491, 734772, 728626, 724061, 720537, 717734, 715451, 713557, 711958, 710592, 709411, 708380, 
  707472, 706666, 705945, 705298, 704713, 704182, 703697, 703253, 702845, 702469, 702121, 701797, 701497, 701216, 700954, 700708, 
  700478, 700260, 700056, 699863, 699680, 699507, 699343, 699187, 699039, 698898, 698763, 698635, 698513, 698395, 698283, 698176, 
  698072, 697973, 697878, 697787, 697699, 697614, 697533, 697454, 697378, 697305, 697234, 697166, 697100, 697036, 696974, 696914
};
int rms_schedule_check(int pid){
  ...
  int numprocs = 0;
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
    if( (p->sched_policy == 1 && p->killed ==0) || (p->pid == pid) ){
      numprocs++;
...
int set_sched_policy(int pid, int policy)
{
  ...
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
    if(p->pid == pid){
      p->start_ticks = ticks;
      if(policy==0){
        if(edf_schedule_check(pid) == -1){
          release(&ptable.lock);
          return -22;
        }
      }
      else if(policy==1){
        if(rms_schedule_check(pid) == -1){
          release(&ptable.lock);
          return -22;
        }
      }
